"""
Faster R-CNN Model Test and Evaluation Script with Robustness Testing
...
def load_class_names(data_root: Path) -> List[str]:
    ...
    if not yaml_path.exists():
        # NEW: Check one level up for data.yaml, useful for robustness test sets
...
# ============================================================
# 📊 EVALUATION FUNCTIONS (Mostly Unchanged)
# ============================================================
@torch.no_grad()
...
# NEW: Function to plot comparison chart
...
# NEW: A reusable function to run the complete evaluation for a given data split
def run_evaluation_on_split(model, device, data_root, split_name, class_names, num_classes, args, output_dir):
    ...
    # Create dedicated output directory for this split
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Load test dataset for the specified split
...
def main(args):
    ...
    # --- 2. Robustness Test Evaluation (if requested) ---
...
if __name__ == '__main__':
    ...
    # NEW: Optional argument for the robustness dataset
